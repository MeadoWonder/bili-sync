...
pub async fn fetch_page_poster(
    ...
    if let Some(fanart_path) = fanart_path {
        if !single_page && cx.config.library_type == LibraryType::Movie {
            cx.downloader
                .fetch(video_model.cover.as_str(), &fanart_path, &cx.config.concurrent_limit.download)
                .await?;
        } else {
...
pub async fn generate_page_nfo(
    ...
    let video_with_page_name = &(video::Model {
        name: format!("P{} {} ✧ {}", page_model.pid, page_model.name, video_model.name),
        ..video_model.clone()
    });
    let single_page = video_model.single_page.context("single_page is null")?;
    let nfo = if single_page {
        ...
        NFO::Movie(video_with_page_name.to_nfo(cx.config.nfo_time_type))
    } else {
